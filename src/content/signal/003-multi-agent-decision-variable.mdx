---
id: sg-003
title: Multi-agent is not an architecture decision. It is a property of the workload.
dek: Anthropic reported a multi-agent system beating single-agent by 90 percent. One day earlier, Cognition told you not to build multi-agents at all. They measured different workloads. Here is the variable that decides, and what it costs to get wrong, measured on 100 queries.
description: "Anthropic and Cognition published opposite conclusions on multi-agent systems within a day of each other in June 2025. Both are right, because they measured different workloads. The decision is not multi-agent yes or no. It is whether the work requires decomposition a router cannot encode. This Signal gives the decision rule, the primary sources on both sides, and a first-party 100-query experiment measuring the cost of getting it wrong."
...
sources:
  ...
  - title: "Multi-agent vs router on 100 customer-support queries"
    url: https://github.com/sunilp/agentic-ai/tree/main/code/labs/lab-001
    authors: [Prakash S.]
    year: 2026
---
...
<Callout type="gotcha">
Router 87 percent vs multi-agent 74 percent on accuracy. After controlling for the multi-agent classifier's own misroutes, the pure architecture cost is 3.4 points. Cost: $0.024 vs $0.057 per query, 2.4 times more expensive. Latency: 1.8s vs 4.0s at p50, 2.2 times slower. The classifier and verifier are net-new model calls; the worker does the same job the router's single agent already did.
</Callout>
...
<Footer3Col
  ...
  inCode={[
    { label: "Lab-001: multi-agent vs router, full harness and data", href: "https://github.com/sunilp/agentic-ai/tree/main/code/labs/lab-001" },
  ]}
