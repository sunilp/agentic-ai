...
[FN-004](/field-notes/004-loop-engineering-30-year-old-loop/) measured why a model is the wrong party to hold this line by itself: without an outside check, asking a model to review its own answer does not just fail to help, it can make things worse. A DeepMind team found intrinsic self-correction, a model revising its own response with no external signal, dropped GPT-4's accuracy on grade-school math from 95.5 to 91.5 percent, because the model talked itself out of correct answers nearly as readily as wrong ones. That is a measurement about correcting a conclusion, not about obeying a standing instruction, but the shape travels one hop over: asking the party that just decided an action is correct to also be the check on that decision is asking the wrong party twice.
