---
description: "Why a subagent must receive strictly fewer permissions than the agent that called it, how authority attenuates at each hop of a delegation chain, and what breaks when a chain re-grants what an earlier hop gave up."
updated: 2026-08-21
readingTime: 7
forces:
...
The rule is simple to state and inconvenient to build: authority narrows at every hop, and never widens. Whatever a subagent receives from its caller must be expressible as a restriction of what the caller held, never as an addition to it.
