...
spot: /assets/spots/lab-001.png
spotCaption: five plain steps outweighed one clever agent
date: 2026-05-20
readingTime: 12
seed: 42
references:
...
- The harness and the sampled query set are not published for this lab. The numbers above are from a real run, but treat them as a directional result, not a reproduction target. A publishable, reproducible version is planned.

```python
# Illustrative harness shape. The harness and query set for this run are not published.
