...
patterns: [approval-gate, tool-agent-registry]
sources:
...
<ArchitectureDiagram
  ...
  caption="Tool execution is the only agent-side chokepoint in both harm families. The prompt layer is in neither."
/>
...
2. **Tool-execution policy.** Every tool call passes a policy decision that knows the actor, the action, and the resource. Deterministic, auditable, and the last agent-side defence against action harm.
