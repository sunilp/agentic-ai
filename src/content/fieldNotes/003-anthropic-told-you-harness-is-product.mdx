...
dek: Three Anthropic posts, one independent essay, and a late-2025 preprint converge on the same point. The unit of evaluation is harness plus model plus task, not model.
description: "Three Anthropic posts and a late-2025 preprint say the same thing. The unit of evaluation is harness plus model plus task, not model alone."
...
The field just renamed a layer that has been there for two years. Anyone who has shipped a tool-using agent has been writing one of these: the runtime that wraps a model, decides what enters the context window, dispatches tool calls, fires hooks, spawns sub-agents, persists memory, and gates permissions. The code was always there. The word for it was not. Through 2025 the discourse called it "the agent loop" or "the scaffolding" or, in the rougher rooms, "the stuff around the model." In the first half of 2026 a single word started showing up in vendor docs, post-mortems, and independent essays at the same time: harness.

Three Anthropic posts from April and May 2026, one independent essay from outside the Anthropic orbit, and a late-2025 preprint on evaluation variance converge on the same point. The unit of evaluation is harness, model, and task together. The single-number model benchmark, considered as a property of the model, is a category error. What follows is the synthesis of those five sources.

## The three Anthropic posts

[Managed Agents](https://www.anthropic.com/engineering/managed-agents) shipped in April 2026 carrying a phrase that did not exist in the public vocabulary three months earlier: harnesses encode assumptions that go stale as models improve. The framing is load-bearing. The vendor that trains the model is positioning itself as a harness vendor whose harness happens to ship with a first-party model. Read literally, the line is a confession that the layer between the model and the user accumulates implicit commitments, and that those commitments rot at a different rate than weights. Read commercially, it is a repositioning. The product surface is the harness. The model is interchangeable inside it. The launch material says so without qualification, and the platform docs repeat the point.
...
Phil Schmid published [The importance of Agent Harness in 2026](https://www.philschmid.de/agent-harness-2026) from outside the Anthropic orbit. The piece argues the same point, in the same vocabulary, without a vendor incentive to do so. That matters. A term that appears only in one vendor's launch material is marketing. A term that appears in independent writing within weeks of the launch has escaped vendor vocabulary into the working language of the field. Schmid's essay reads as confirmation that the layer is now named for everyone, not just for buyers of one stack.
...
The three Anthropic posts name the layer and trace product behavior to it. The independent essay confirms the term is now common vocabulary. The preprint shows the variance the layer introduces is large enough to matter. Five sources, one shape.
...
**Tool dispatch.** Which tools the model can see, in what order, with which schemas, with which parallelism, and with which error semantics on failure. This is the most visible part of the harness and the part that shifts most often between releases. Chapter 2 covers the contract shape and the failure modes that come from sloppy dispatch.
...
Six components, six policy surfaces, six places a vendor can change behavior without touching weights. Chapter 13 walks through the cross-cutting production picture once the components are in place.
...
The three Anthropic posts demonstrate the harness term operationally. The April 23 post-mortem proves the harness alone can swing perceived quality enough to generate weeks of community complaint. The preprint proves the run-to-run variance is large enough to swamp small differences between systems. Together they argue that a single-number model benchmark, read as a property of the model, is reading sales material as evidence. The number is real. What it measures is not the model.

What disclosure should look like in research that wants to be reproducible: the harness identity and version, the configuration applied to each of the six components, the seed protocol, the number of runs, and the variance across them. What benchmarks should report and almost never do: any of the above. Promptfoo's [evaluation guidance for coding agents](https://www.promptfoo.dev/docs/guides/evaluate-coding-agents/) names the problem in the title. The framing is correct. The industry's reporting practice has not caught up.
