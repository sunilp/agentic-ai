---
id: r-001
title: Build and deploy your first Strands agent on AgentCore Runtime
...
chapter: ch-09
spot: /assets/spots/r-001.png
spotCaption: deployed, verified, stamped
prerequisites:
...
verifiedVersions:
  'AWS CLI': '2.34.45'
  'Node': '23.7.0'
  '@aws/agentcore': '0.13.1'
  'CDK': '2.1100.1'
  'Region': 'us-east-1'
  'Model': 'global.anthropic.claude-sonnet-4-6'
runItRepo: https://github.com/sunilp/agentic-ai/tree/main/code/recipes/r-001
gotchas:
  - 'Region matters: AgentCore is GA in us-east-1, us-west-2, eu-central-1, ap-southeast-1.'
  - 'Model use-case form must be submitted before the first invoke or you get a ResourceNotFoundException.'
  - 'CDK bootstrap is required if you have never used CDK in this account+region (pass --yes to agentcore deploy to handle it automatically).'
  - 'Some Claude model IDs route through AWS Marketplace and require aws-marketplace:Subscribe on the invoking principal — use global.anthropic.claude-sonnet-4-6 to avoid this.'
  - 'The scaffold creates aws-targets.json as empty [] — you must add a target manually before deploying.'
references:
  - ch-09
  - ch-11
patterns:
...
<Callout type="gotcha">
The CLI does not auto-populate `agentcore/aws-targets.json` on `create`. Add one yourself before deploying. The CLI does not error early — it gets through validate and synth before complaining about missing targets.
</Callout>
